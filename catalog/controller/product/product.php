...
$data['href1'] = $this->url->link('product/product', $url . '&product_id=' . $this->request->get['product_id']);
			$data['breadcrumbs'][] = array(
			...
			$sprice = (int)$product_info['special'];
			$prr = (int)$product_info['price'];
			$ss = $prr - $sprice;


			$p = (($prr - $sprice) / $prr) * 100;
			$data['save'] = $ss;
			$data['percent1'] = (number_format((float)$p, 0, '.', ''));
			...
			if ($information_info) {
				$data['product_delivery'] = html_entity_decode($information_info['description'], ENT_QUOTES, 'UTF-8');
			} else {
			}
			...
			foreach ($results as $result) {
				...
				if (!is_null($result['special']) && (float)$result['special'] >= 0) {
					...
					$percent1 = (($result['price'] - $result['special']) / $result['price']) * 100;
...
// 			echo "</pre>";
			...
			$data['occasion'] = $product_info['upc'];
			$data['pattern'] = $product_info['ean'];
			$data['fabric'] = $product_info['jan'];
			$data['sku_id'] = $product_info['sku'];
			$data['sleeve'] = $product_info['isbn'];

			$data['washcare'] = $product_info['mpn'];
			$data['style'] = $product_info['location'];
			$data['care'] = $product_info['meta_description'];
